...
| الملف | المحتوى |
|-------|---------|
| `index.ts` | المدخل — Webhook + Cron + Auth (WEBHOOK_SECRET) |
| `utils.ts` | دوال Telegram API + Supabase Clients + fmtDate() + STATUS_LABEL |
| `commands.ts` | 14 أمر نصي (/help, /orders, /today, /stock, ...) |
| `callbacks.ts` | معالج الأزرار: `wf_*` (فريق) + `inv_*` (مخزون) + `order_*` (قديم) |
| `incoming.ts` | استقبال فواتير POS من المخزون + إرسالها للتليجرام |
...
بعد أي تغيير جوهري في النظام، حدّث هذا الملف. آخر تحديث: 2026-05-22.
